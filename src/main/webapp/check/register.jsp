	...
	<script type="text/javascript">
		...
		function checkPassword_again(event) {
			...
			$.get('inspectTheSamePassword.do', {
				...
				$('#' + mPassword + '_again').parent().find('span')
						.text(output);
			});
		}
		...
		// 自動驗證請再輸入一次密碼是否相同
		$('#mPassword_again').on('keyup', checkPassword_again).on('blur',
		...
		$(document).ready(
				function() {
					var idCheckArray = [ 'mEmail', 'mPassword',
							'mPassword_again', 'mName', 'mIdentityCardNumber',
		...
		$(document)
				.ready(
						function() {
							...
							$(
									'#mAddress_Region option[value="'
											+ mAddress_Region + '"]').prop(
									'selected', true);
						});
	</script>
</body>
