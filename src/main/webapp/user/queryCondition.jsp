...
<body>
				...
				<tbody>
					<tr>
						...
						<td><input type="checkbox" name="mI_L1" value="${user.i_d.items.i1.key}" />${user.i_d.items.i1.value}<input
							type="checkbox" name="mI_L2" value="${user.i_d.items.i2.key}" />${user.i_d.items.i2.value}<input type="checkbox"
							name="mI_L3" value="${user.i_d.items.i3.key}" />${user.i_d.items.i3.value}<input type="checkbox" name="mI_L4"
							value="${user.i_d.items.i4.key}" />${user.i_d.items.i4.value}<input type="checkbox" name="mI_L5"
							value="${user.i_d.items.i5.key}" />${user.i_d.items.i5.value}<input type="checkbox" name="mI_L6"
							value="${user.i_d.items.i6.key}" />${user.i_d.items.i6.value}<input type="checkbox" name="mI_L7"
							value="${user.i_d.items.i7.key}" />${user.i_d.items.i7.value}<input type="checkbox" name="mI_L8"
	...
	<script type="text/javascript">
		...
		var address_TaiwanArray = [ '皆可', '臺北市', '基隆市', '新北市', '桃園市', '新竹市',
				'新竹縣', '苗栗縣', '臺中市', '彰化縣', '南投縣', '雲林縣', '嘉義市', '嘉義縣', '臺南市',
		...
		var address_AllArray = [ address_EmptyArray, address_TaipeiCityArray,
		...
		function changeOptionOfmAddress_Region() {
			...
			for (var i = 0; i < regionArray.length; i++) {
				var checkboxJOb = $('<input type="checkbox" id="mA_RL' + i + '" name="mA_RL' + i + '" value="' + regionArray[i] + '">'
						+ regionArray[i] + '</input>');
				var spanJOb;
				if (i % 10 == 9) {
					spanJOb = $('<br />');
		...
		$('#mOtherInterest').on('keydown', checkInterest).on('blur',
				checkInterestOnblur);

		function removeDivAndRearrangeAllInput(event) {
			// 先找到父元素，再馬上移除該div，否則影響到div的剩餘數目。
			var inputJOb = $(event.target).parents('tr').find(
					'input[type="text"]');
			$(event.target).parent('div').remove();
			var otherInterestDivJOb = inputJOb.parent('tr').find(
		...
		function createOtherInterestDiv(event) {
			...
			var otherInterestDivJOb = inputJOb.parents('tr').find(
					'div[class="otherInterestDiv"]');
			var innerDivs = otherInterestDivJOb.children();
			if (inputJOb.val() == '') {
				return;
			...
			if (innerDivs.length < 5) {
				...
				var cell4 = $(
						'<input type="hidden" name="mOI_L'
								+ (innerDivs.length + 1) + '" />').val(
						inputJOb.val());
		...
		$(document)
				.ready(
						function() {
							...
							var mAddress_RegionCheckedArray = ("${mAddress_RegionCheckedArray}")
									.split(',');
							var checkboxJObs = $('#mAddress_Region').children(
									'input[type="checkbox"]');
		...
		$(document).ready(
				function() {
					for (var i = 0; i < mInterestExistArray.length; i++) {
						var checkboxJOb;
						for (var j = 1; j < 64; j++) {
							checkboxJOb = $('#interestTable').find(
									'input[name="mI_L' + j + '"]');
		...
		var mOtherInterestExistArray = ("${mOtherInterestExistArray}")
				.split(',');
