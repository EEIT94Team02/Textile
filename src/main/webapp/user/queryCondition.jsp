...
<body>
	<form id="queryCondition" action="queryCondition.do" method="post">
		...
		<div>
			...
			<c:set var="genderWords" value="無,男,女" />
		...
		<div>
			<label for="mAddress_County">地區：</label> <select id="mAddress_County" name="mAddress_County">
				<c:set var="address_Taiwan"
					value="無,臺北市,基隆市,新北市,桃園市,新竹市,新竹縣,苗栗縣,臺中市,彰化縣,南投縣,雲林縣,嘉義市,嘉義縣,臺南市,高雄市,屏東縣,宜蘭縣,花蓮縣,臺東縣,澎湖縣,金門縣,連江縣" />
				<c:forEach items="${address_Taiwan.split(',')}" var="address_TaiwanArray">
					<option value="${address_TaiwanArray}" ${dataAndErrorsMap.mAddress_County == address_TaiwanArray ? 'selected' : ''}>${address_TaiwanArray}</option>
				</c:forEach>
			</select>
			<p></p>
		...
		<div>
			...
			<label for="mScores">積分：</label> <input type="number" id="mScores" name="mScores" min="0" max="10000000" value="${x}" /><br />
			<p></p>
		</div>
		<div>
			<c:choose>
		...
		<div>
			<table>
				<tbody>
					<tr>
						<td>戶外休閒：</td>
						<td><input type="checkbox" name="i1" value="${user.i_d.items.i1.key}" />${user.i_d.items.i1.value}<input
							type="checkbox" name="i2" value="${user.i_d.items.i2.key}" />${user.i_d.items.i2.value}<input type="checkbox"
							name="i3" value="${user.i_d.items.i3.key}" />${user.i_d.items.i3.value}<input type="checkbox" name="i4"
							value="${user.i_d.items.i4.key}" />${user.i_d.items.i4.value}<input type="checkbox" name="i5"
							value="${user.i_d.items.i5.key}" />${user.i_d.items.i5.value}<input type="checkbox" name="i6"
							value="${user.i_d.items.i6.key}" />${user.i_d.items.i6.value}<input type="checkbox" name="i7"
							value="${user.i_d.items.i7.key}" />${user.i_d.items.i7.value}<input type="checkbox" name="i8"
							value="${user.i_d.items.i8.key}" />${user.i_d.items.i8.value}</td>
					</tr>
					<tr>
						<td>運動健身：</td>
						<td><input type="checkbox" name="i9" value="${user.i_d.items.i9.key}" />${user.i_d.items.i9.value}<input
							type="checkbox" name="i10" value="${user.i_d.items.i10.key}" />${user.i_d.items.i10.value}<input type="checkbox"
							name="i11" value="${user.i_d.items.i11.key}" />${user.i_d.items.i11.value}<input type="checkbox" name="i12"
							value="${user.i_d.items.i12.key}" />${user.i_d.items.i12.value}<input type="checkbox" name="i13"
							value="${user.i_d.items.i13.key}" />${user.i_d.items.i13.value}<input type="checkbox" name="i14"
							value="${user.i_d.items.i14.key}" />${user.i_d.items.i14.value}<input type="checkbox" name="i15"
							value="${user.i_d.items.i15.key}" />${user.i_d.items.i15.value}<input type="checkbox" name="i16"
							value="${user.i_d.items.i16.key}" />${user.i_d.items.i16.value}
						<td></td>
					</tr>
					<tr>
						<td>飲食料理：</td>
						<td><input type="checkbox" name="i17" value="${user.i_d.items.i17.key}" />${user.i_d.items.i17.value}<input
							type="checkbox" name="i18" value="${user.i_d.items.i18.key}" />${user.i_d.items.i18.value}<input type="checkbox"
							name="i19" value="${user.i_d.items.i19.key}" />${user.i_d.items.i19.value}<input type="checkbox" name="i20"
							value="${user.i_d.items.i20.key}" />${user.i_d.items.i20.value}<input type="checkbox" name="i21"
							value="${user.i_d.items.i21.key}" />${user.i_d.items.i21.value}<input type="checkbox" name="i22"
							value="${user.i_d.items.i22.key}" />${user.i_d.items.i22.value}<input type="checkbox" name="i23"
							value="${user.i_d.items.i23.key}" />${user.i_d.items.i23.value}<input type="checkbox" name="i24"
							value="${user.i_d.items.i24.key}" />${user.i_d.items.i24.value}
						<td></td>
					</tr>
					<tr>
						<td>文學藝術：</td>
						<td><input type="checkbox" name="i25" value="${user.i_d.items.i25.key}" />${user.i_d.items.i25.value}<input
							type="checkbox" name="i26" value="${user.i_d.items.i26.key}" />${user.i_d.items.i26.value}<input type="checkbox"
							name="i27" value="${user.i_d.items.i27.key}" />${user.i_d.items.i27.value}<input type="checkbox" name="i28"
							value="${user.i_d.items.i28.key}" />${user.i_d.items.i28.value}<input type="checkbox" name="i29"
							value="${user.i_d.items.i29.key}" />${user.i_d.items.i29.value}<input type="checkbox" name="i30"
							value="${user.i_d.items.i30.key}" />${user.i_d.items.i30.value}<input type="checkbox" name="i31"
							value="${user.i_d.items.i31.key}" />${user.i_d.items.i31.value}<input type="checkbox" name="i32"
							value="${user.i_d.items.i32.key}" />${user.i_d.items.i32.value}
						<td></td>
					</tr>
					<tr>
						<td>設計藝術：</td>
						<td><input type="checkbox" name="i33" value="${user.i_d.items.i33.key}" />${user.i_d.items.i33.value}<input
							type="checkbox" name="i34" value="${user.i_d.items.i34.key}" />${user.i_d.items.i34.value}<input type="checkbox"
							name="i35" value="${user.i_d.items.i35.key}" />${user.i_d.items.i35.value}<input type="checkbox" name="i36"
							value="${user.i_d.items.i36.key}" />${user.i_d.items.i36.value}<input type="checkbox" name="i37"
							value="${user.i_d.items.i37.key}" />${user.i_d.items.i37.value}<input type="checkbox" name="i38"
							value="${user.i_d.items.i38.key}" />${user.i_d.items.i38.value}<input type="checkbox" name="i39"
							value="${user.i_d.items.i39.key}" />${user.i_d.items.i39.value}<input type="checkbox" name="i40"
							value="${user.i_d.items.i40.key}" />${user.i_d.items.i40.value}
						<td></td>
					</tr>
					<tr>
						<td>音樂藝術：</td>
						<td><input type="checkbox" name="i41" value="${user.i_d.items.i41.key}" />${user.i_d.items.i41.value}<input
							type="checkbox" name="i42" value="${user.i_d.items.i42.key}" />${user.i_d.items.i42.value}<input type="checkbox"
							name="i43" value="${user.i_d.items.i43.key}" />${user.i_d.items.i43.value}<input type="checkbox" name="i44"
							value="${user.i_d.items.i44.key}" />${user.i_d.items.i44.value}<input type="checkbox" name="i45"
							value="${user.i_d.items.i45.key}" />${user.i_d.items.i45.value}<input type="checkbox" name="i46"
							value="${user.i_d.items.i46.key}" />${user.i_d.items.i46.value}<input type="checkbox" name="i47"
							value="${user.i_d.items.i47.key}" />${user.i_d.items.i47.value}<input type="checkbox" name="i48"
							value="${user.i_d.items.i48.key}" />${user.i_d.items.i48.value}
						<td></td>
					</tr>
					<tr>
						<td>興趣嗜好：</td>
						<td><input type="checkbox" name="i49" value="${user.i_d.items.i49.key}" />${user.i_d.items.i49.value}<input
							type="checkbox" name="i50" value="${user.i_d.items.i50.key}" />${user.i_d.items.i50.value}<input type="checkbox"
							name="i51" value="${user.i_d.items.i51.key}" />${user.i_d.items.i51.value}<input type="checkbox" name="i52"
							value="${user.i_d.items.i52.key}" />${user.i_d.items.i52.value}<input type="checkbox" name="i53"
							value="${user.i_d.items.i53.key}" />${user.i_d.items.i53.value}<input type="checkbox" name="i54"
							value="${user.i_d.items.i54.key}" />${user.i_d.items.i54.value}<input type="checkbox" name="i55"
							value="${user.i_d.items.i55.key}" />${user.i_d.items.i55.value}<input type="checkbox" name="i56"
							value="${user.i_d.items.i56.key}" />${user.i_d.items.i56.value}
						<td></td>
					</tr>
					<tr>
						<td>其它活動：</td>
						<td><input type="checkbox" name="i57" value="${user.i_d.items.i57.key}" />${user.i_d.items.i57.value}<input
							type="checkbox" name="i58" value="" />${user.i_d.items.i58.value}<input type="checkbox" name="i59"
							value="${user.i_d.items.i59.key}" />${user.i_d.items.i59.value}<input type="checkbox" name="i60"
							value="${user.i_d.items.i60.key}" />${user.i_d.items.i60.value}<input type="checkbox" name="i61"
							value="${user.i_d.items.i61.key}" />${user.i_d.items.i61.value}<input type="checkbox" name="i62"
							value="${user.i_d.items.i62.key}" />${user.i_d.items.i62.value}<input type="checkbox" name="i63"
							value="${user.i_d.items.i63.key}" />${user.i_d.items.i63.value}<input type="checkbox" name="i64"
							value="${user.i_d.items.i64.key}" />${user.i_d.items.i64.value}
						<td></td>
					</tr>
					<tr>
	...
	<c:if test="${not empty mInterest_List}">
		<c:set var="start" value="${false}" />
		<c:forEach var="mInterest_List" items="${mInterest_List}" varStatus="status">
	...
	<c:if test="${not empty mOtherInterestList}">
		<c:set var="start" value="${false}" />
		<c:forEach var="mOtherInterestArray" items="${mOtherInterestList}" varStatus="status">
    ...
    // 縣市鄉鎮區的陣列，因為可以不選，故新增「無」。
    var address_TaiwanArray = ['無', '臺北市', '基隆市', '新北市', '桃園市', '新竹市', '新竹縣', '苗栗縣', '臺中市', '彰化縣', '南投縣', '雲林縣', '嘉義市', '嘉義縣', '臺南市', '高雄市', '屏東縣', '宜蘭縣', '花蓮縣', '臺東縣', '澎湖縣', '金門縣', '連江縣'];
    ...
    function createOtherInterestDiv(event) {
      ...
      if (innerDivs.length < 5) {
        ...
        var cell4 = $('<input type="hidden" name="o' + (innerDivs.length + 1) + '" />').val(inputJOb.val());
        var newDivJOb = $('<div></div>').append([cell1, cell2, cell3, cell4]);
        console.log(newDivJOb);
