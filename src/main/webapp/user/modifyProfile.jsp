...
<body>
			...
			<form id="profile" action="modify.do" method="post">
				...
				<fieldset>
					...
					<div>
						...
						<label for="mBirthday">生日：</label> <input id="mBirthday" name="mBirthday"
							value="${fn:substring(x.toString(),0,10)}" type="text" size="21" maxlength="10" placeholder="2000-2-29" required />
						<img src="" /><span>${dataAndErrorsMap.mBirthday_error}</span> <br />
					...
					<div>
						...
						<input id="mAddress" name="mAddress" value="${x}" type="text" size="65" maxlength="60" placeholder="" required />
						<img src="" /><span>${dataAndErrorsMap.mAddress_error}</span> <br />
						<p></p>
					</div>
					<div>
						<c:choose>
					...
					<div>
						...
						<label for="mHintAnswer">提示答案：</label> <input id="mHintAnswer" name="mHintAnswer" value="${x}" type="text"
							size="45" maxlength="40" placeholder="" required /> <img src="" /><span>${dataAndErrorsMap.mHintAnswer_error}</span>
						<br />
						<p></p>
					</div>
					<div>
	...
	<script type="text/javascript">
		...
		// 自動驗證身分證字號
		$('#mIdentityCardNumber').on('keyup', checkField).on('blur',
				checkFieldOnblur);
		// 自動驗證生日
		...
		// 自動驗證密碼提示
		$('#mHintPassword').on('keyup', checkField)
				.on('blur', checkFieldOnblur);
		// 自動驗證提示答案
		...
		$(document).ready(
				function() {
					var idCheckArray = [ 'mName', 'mIdentityCardNumber',
							'mBirthday', 'mPhoneNumber', 'mAddress',
							'mHintPassword', 'mHintAnswer' ];
		...
		$(document).ready(
				function() {
					...
					$(
							'#mAddress_Region option[value="' + mAddress_Region
									+ '"]').prop('selected', true);
				});
	</script>
</body>
